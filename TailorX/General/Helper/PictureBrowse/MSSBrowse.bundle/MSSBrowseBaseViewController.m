...
/** 导航条*/
@property (nonatomic, strong) TXNavigationViewController *nav;
...
- (void)dealloc
{
    NSLog(@"MSSBrowseBaseViewController <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<");
...
- (void)showBrowseViewController
{
    ...
    self.nav = [[TXNavigationViewController alloc]initWithRootViewController:self];
...
- (void)browseActionSheetDidSelectedAtIndex:(NSInteger)index currentCell:(MSSBrowseCollectionViewCell *)currentCell
{
    if (self.isDelete) {
        ...
        if(index == 0)
        {
            NSMutableArray *tempArr = [NSMutableArray arrayWithArray:self.browseItemArray];
            [tempArr removeObjectAtIndex:indexPath.row];
            self.browseItemArray = tempArr;
