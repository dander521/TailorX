...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(cell)
    {
        ...
        [cell longPress:^(MSSBrowseCollectionViewCell *browseCell) {
            __strong __typeof(weakSelf)strongSelf = weakSelf;
            if ([NSString isTextEmpty:browseItem.bigImageUrl]) {
                // 本地图片无需保存
            }else {
                [[EMSDImageCache sharedImageCache]diskImageExistsWithKey:browseItem.bigImageUrl completion:^(BOOL isInCache) {
...
- (void)tap:(MSSBrowseCollectionViewCell *)browseCell {
    ...
    if (self.dismissBlock) {
        self.dismissBlock([NSIndexPath indexPathForRow:indexPath.row inSection:0]);
    }
    // 移除通知
    ...
    browseCell.zoomScrollView.zoomScale = 1.f;
