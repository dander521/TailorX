...
+ (void)textView:(id)view maxLength:(NSInteger)length showEmoji:(BOOL) isShowEmoji{
...
-(UIColor *)loadThemeColorWithName:(NSString *)colorName {
    ...
    NSDictionary *fontColorDic = [NSDictionary dictionaryWithContentsOfFile:fontFilePath];
    NSDictionary *rbgDic = fontColorDic[colorName];
    if (rbgDic.count < 3) {
        return nil;
    }
...
-(NSString *)themePath{
    ...
    NSDictionary *theDic = [NSDictionary dictionaryWithContentsOfFile:filePath];
    NSString *themePath = theDic[@"black"];
