...
//  TailorX
//
//  Created by Qian Shen on 2017/4/6.
...
@interface TXGuideView ()

/** titels*/
@property (nonatomic, strong) NSArray *titels;
/** 图片*/
...
- (void)refreshVisibleCellAppearance{
    ...
    switch (self.orientation) {
        case TXGuideViewOrientationHorizontal:{
            ...
            for (NSInteger i = self.visibleRange.location; i < self.visibleRange.location + _visibleRange.length; i++) {
                ...
                if (delta < _pageSize.width) {
                    CGFloat inset = (_pageSize.width * (1 - 0.92)) * (delta / _pageSize.width)/2.0;
                    cell.frame = UIEdgeInsetsInsetRect(originCellFrame, UIEdgeInsetsMake(inset, inset, inset, inset));
                } else {
                    CGFloat inset = _pageSize.width * (1 - 0.92) / 2.0 ;
                    cell.frame = UIEdgeInsetsInsetRect(originCellFrame, UIEdgeInsetsMake(inset, inset, inset, inset));
                }
            }
            break;
        ...
        case TXGuideViewOrientationVertical:{
            ...
            for (NSInteger i = self.visibleRange.location; i < self.visibleRange.location + _visibleRange.length; i++) {
                TXGuideItemView *cell = [_cells objectAtIndex:i];
...
- (void)reloadData
{
    ...
    if (_needsReload) {
        ...
        switch (self.orientation) {
            case TXGuideViewOrientationHorizontal://横向
...
- (UIView *)dequeueReusableCell{
    TXGuideItemView *cell = [_reusableCells lastObject];
    if (cell) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    // 处理引导页 第一张 不可回滑
    if (scrollView.contentOffset.x < 3*(SCREEN_WIDTH-100) && self.isCarousel == true) {
        [scrollView setContentOffset:CGPointMake(3*(SCREEN_WIDTH-100), scrollView.contentOffset.y) animated:NO];
        return;
    }
    NSInteger pageIndex;
    switch (self.orientation) {
