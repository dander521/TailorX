...
- (void)setupTitles {
    ...
    weakSelf(self);
    self.titleBadgeNumber = ^(NSInteger badgeNum, NSInteger indexInput) {
        for (ZJTitleView *titleView in weakSelf.titleViews) {
            if (titleView.tag == indexInput) {
                titleView.badgeNum = badgeNum;
                break;
            }
        }
    };
