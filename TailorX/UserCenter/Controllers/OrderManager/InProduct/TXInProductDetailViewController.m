...
@interface TXInProductDetailViewController () <UITableViewDelegate, UITableViewDataSource, TXOrderTableViewCellDelegate, NetErrorViewDelegate, TXOrderHeaderViewDelegate>
/** TableView */
...
@implementation TXInProductDetailViewController
...
- (void)setUpContentTableView {
    self.queueTableView = ({
        UITableView *tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0.01, SCREEN_WIDTH, SCREEN_HEIGHT-50.0-kTopHeight) style:UITableViewStyleGrouped];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0) {
        ...
        headerCell.headerDesLabel.text = @"";
        headerCell.headerStatusLabel.text = self.orderDetail.factoryTacheName;
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    if (section == 2) {
        ...
        headerView.rightStatusLabel.text = self.orderDetail.sortNo == nil ? @"" : [NSString stringWithFormat:@"%@号", self.orderDetail.sortNo];
        return headerView;
    }
    return nil;
}
...
- (UIView *)bottomView {
    if (!_bottomView) {
        ...
        [contactStoreBtn addTarget:self action:@selector(touchContactStore) forControlEvents:UIControlEventTouchUpInside];
