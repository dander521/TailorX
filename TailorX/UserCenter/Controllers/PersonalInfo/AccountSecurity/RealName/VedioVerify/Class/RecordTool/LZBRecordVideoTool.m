...
-(void)captureOutput:(AVCaptureFileOutput *)captureOutput didFinishRecordingToOutputFileAtURL:(NSURL *)outputFileURL fromConnections:(NSArray *)connections error:(NSError *)error{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(videoToolCaptureOutput:didFinishRecordingToOutputFileAtURL:fromConnections:error:)]) {
        
        [self.delegate videoToolCaptureOutput:captureOutput didFinishRecordingToOutputFileAtURL:outputFileURL fromConnections:connections error:error];
    }
    NSLog(@"视频录制完成.");
    //视频录入完成之后在后台将视频存储到相
...
- (AVCaptureDeviceInput *)backCameraInput {
    if (_backCameraInput == nil) {
        ...
        if (error) {
            [MBProgressHUD showError:@"获取摄像头失败，请检查是否开启摄像头权限"];
...
- (AVCaptureDeviceInput *)audioMicInput {
    if (_audioMicInput == nil) {
        ...
        if (error) {
            [MBProgressHUD showError:(@"获取摄像头失败，请检查是否开启麦克风权限")];
