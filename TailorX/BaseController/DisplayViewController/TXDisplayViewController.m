...
@interface TXDisplayViewController ()<UICollectionViewDataSource,UICollectionViewDelegate>
...
/** 颜色渐变样式*/
@property (nonatomic, assign) TXTitleColorGradientStyle titleColorGradientStyle;
/** 字体缩放比例*/
...
@implementation TXDisplayViewController
...
- (void)initial {
    // 初始化标题高度
    _titleHeight = TXTitleScrollViewH;
...
- (UIScrollView *)contentScrollView {
    if (_contentScrollView == nil) {
        ...
        // 创建布局
        TXFlowLayout *layout = [[TXFlowLayout alloc] init];
        
        UICollectionView *contentScrollView = [[UICollectionView alloc] initWithFrame:CGRectZero
...
- (void)setIsShowTitleScale:(BOOL)isShowTitleScale {
    if (_isShowUnderLine) {
        // 抛异常
        NSException *excp = [NSException exceptionWithName:@"TXDisplayViewControllerException" reason:@"字体放大效果和角标不能同时使用。" userInfo:nil];
        [excp raise];
    }
...
- (void)setIsfullScreen:(BOOL)isfullScreen {
    ...
    self.contentView.frame = CGRectMake(0, 0, TXScreenW, TXScreenH);
...
- (void)setUpTitleGradient:(void(^)(TXTitleColorGradientStyle *titleColorGradientStyle,UIColor **norColor,UIColor **selColor))titleGradientBlock {
    ...
    if (_titleColorGradientStyle == TXTitleColorGradientStyleFill && _titleWidth > 0) {
...
- (void)viewDidLayoutSubviews {
    ...
    if (_isInitial == NO) {
        ...
        CGFloat titleY = self.navigationController.navigationBarHidden == NO ?TXNavBarH:statusH;
        ...
        if (_isfullScreen) {
            ...
            // 顶部标题View尺寸
            self.titleScrollView.frame = CGRectMake(0, titleY, TXScreenW, self.titleHeight);
        ...
        if (self.contentView.frame.size.height == 0) {
            self.contentView.frame = CGRectMake(0, titleY, TXScreenW, TXScreenH - titleY-TXTabBarH);
        }
        
        // 顶部标题View尺寸
        self.titleScrollView.frame = CGRectMake(0, 0, TXScreenW-40, self.titleHeight);
        
        // 顶部内容View尺寸
        CGFloat contentY = CGRectGetMaxY(self.titleScrollView.frame);
        CGFloat contentH = self.contentView.height - contentY;
        self.contentScrollView.frame = CGRectMake(0, contentY, TXScreenW, contentH);
        [self setUpAddBtn];
    }
...
- (void)setUpAddBtn {
    ...
    addBtn.frame = CGRectMake(SCREEN_WIDTH-44,0, 44, 44);
    [addBtn setImage:[UIImage imageNamed:@"加号"] forState:UIControlStateNormal];
    [self.contentView addSubview:addBtn];
    [addBtn addTarget:self action:@selector(respondsToAddBtn:) forControlEvents:UIControlEventTouchUpInside];
}
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (_isInitial == NO) {
        ...
        if (_titleColorGradientStyle == TXTitleColorGradientStyleFill || _titleWidth == 0) { // 填充样式才需要这样
...
- (void)setUpTitleWidth {
    ...
    CGFloat titleMargin = (TXScreenW - totalWidth) / (count + 1);
...
- (void)setUpAllTitle {
    ...
    for (int i = 0; i < count; i++) {
        ...
        UILabel *label = [[TXDisplayTitleLabel alloc] init];
        ...
        if (_titleColorGradientStyle == TXTitleColorGradientStyleFill || _titleWidth == 0) { // 填充样式才需要
    ...
    _contentScrollView.contentSize = CGSizeMake(count * TXScreenW, 0);
    
}

#pragma mark - 标题效果渐变方法
// 设置标题颜色渐变
- (void)setUpTitleColorGradientWithOffset:(CGFloat)offsetX rightLabel:(TXDisplayTitleLabel *)rightLabel leftLabel:(TXDisplayTitleLabel *)leftLabel {
    if (_isShowTitleGradient == NO) return;
    
    // 获取右边缩放
    CGFloat rightSacle = offsetX / TXScreenW - leftLabel.tag;
    
    // 获取左边缩放比例
    CGFloat leftScale = 1 - rightSacle;
    
    // RGB渐变
    if (_titleColorGradientStyle == TXTitleColorGradientStyleRGB) {
    ...
    // 填充渐变
    if (_titleColorGradientStyle == TXTitleColorGradientStyleFill) {
...
- (void)setUpTitleScaleWithOffset:(CGFloat)offsetX rightLabel:(UILabel *)rightLabel leftLabel:(UILabel *)leftLabel {
    ...
    CGFloat scaleTransform = _titleScale?_titleScale:TXTitleTransformScale;
...
- (void)setUpUnderLineOffset:(CGFloat)offsetX rightLabel:(UILabel *)rightLabel leftLabel:(UILabel *)leftLabel {
    ...
    // 计算当前下划线偏移量
    CGFloat underLineTransformX = offsetDelta * centerDelta / TXScreenW;
    
    // 宽度递增偏移量
    CGFloat underLineWidth = offsetDelta * widthDelta / TXScreenW;
...
- (void)setUpCoverOffset:(CGFloat)offsetX rightLabel:(UILabel *)rightLabel leftLabel:(UILabel *)leftLabel {
    ...
    // 计算当前下划线偏移量
    CGFloat coverTransformX = offsetDelta * centerDelta / TXScreenW;
    
    // 宽度递增偏移量
    CGFloat coverWidth = offsetDelta * widthDelta / TXScreenW;
...
- (void)titleClick:(UITapGestureRecognizer *)tap {
    ...
    if (vc.view) {
        // 发出通知点击标题通知
        [[NSNotificationCenter defaultCenter] postNotificationName:TXDisplayViewClickOrScrollDidFinshNote  object:vc];
        
        // 发出重复点击标题通知
        if (_selIndex == i) {
            [[NSNotificationCenter defaultCenter] postNotificationName:TXDisplayViewRepeatClickTitleNote object:vc];
        }
    }
...
- (void)selectLabel:(UILabel *)label {
    ...
    for (TXDisplayTitleLabel *labelView in self.titleLabels) {
        ...
        if (_isShowTitleGradient && _titleColorGradientStyle == TXTitleColorGradientStyleFill) {
...
- (void)setUpUnderLine:(UILabel *)label {
    ...
    CGFloat underLineH = _underLineH?_underLineH:TXUnderLineH;
...
- (void)setLabelTitleCenter:(UILabel *)label {
    ...
    // 设置标题滚动区域的偏移量
    CGFloat offsetX = label.center.x - TXScreenW * 0.5;
    ...
    // 计算下最大的标题视图滚动区域
    CGFloat maxOffsetX = self.titleScrollView.contentSize.width - TXScreenW + _titleMargin + 30;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    vc.view.frame = CGRectMake(0, 0, self.contentScrollView.width, self.contentScrollView.
                               height);
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    NSInteger screenWInt = TXScreenW;
    
    NSInteger extre = offsetXInt % screenWInt;
    if (extre > TXScreenW * 0.5) {
        // 往右边移动
        offsetX = offsetX + (TXScreenW - extre);
        _isAniming = YES;
        [self.contentScrollView setContentOffset:CGPointMake(offsetX, 0) animated:YES];
    }else if (extre < TXScreenW * 0.5 && extre > 0) {
        _isAniming = YES;
        // 往左边移动
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    // 获取左边角标
    NSInteger leftIndex = offsetX / TXScreenW;
    
    // 左边按钮
    TXDisplayTitleLabel *leftLabel = self.titleLabels[leftIndex];
    
    // 右边角标
    NSInteger rightIndex = leftIndex + 1;
    
    // 右边按钮
    TXDisplayTitleLabel *rightLabel = nil;
