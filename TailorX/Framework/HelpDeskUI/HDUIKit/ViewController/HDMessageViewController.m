...
- (void)viewDidLoad {
    ...
    _cuurenTime = [[NSDate date] timeIntervalSince1970];
    ...
    HConversation *conversation = [[HChatClient sharedClient].chat getConversation:KF_IMId];
    [conversation loadMessagesStartFromId:nil count:2 searchDirection:HMessageSearchDirectionUp completion:^(NSArray *aMessages, HError *aError) {
        if (!aMessages || aMessages.count == 0) {
            EMTextMessageBody *textBody = [[EMTextMessageBody alloc] initWithText:@"您好！这里是TailorX官方客服，请问需要什么帮助？"];
            HMessage *message = [[HMessage alloc] initWithConversationID:self.conversation.conversationId from:@"TailorX客服" to:[NSString stringWithFormat:@"tailorx%@",GetUserInfo.accountA] body:textBody];
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    // 新消息提醒
    [[NSNotificationCenter defaultCenter]postNotificationName:kNSNotificationFindNewServiceMessage object:@(0)];
    SaveUserInfo(isUnreadMessages, NO);
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([object isKindOfClass:[NSString class]]) {
        ...
        if (model.isSender) {
            model.avatarURLPath = GetUserInfo.photo;
                     ...
                     progress:(id)progress
{
    ...
    dispatch_async(_messageQueue, ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [weakSelf.tableView scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:[weakSelf.dataArray count] - 1 inSection:0] atScrollPosition:UITableViewScrollPositionBottom animated:true];
        });
    });
}
